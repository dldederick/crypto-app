...
//         this.state.updatedAssetList.map(async (obj) => {
//           const purchaseDate = convertToUnixTimestamp(obj.assetPurchaseDate);
//           const newDate = Math.floor(Date.now() / 1000);
//           const name = obj.id;
//           const currency = this.props.selectedCurrency;
//           const { data } = await axios(`https://api.coingecko.com/api/v3/coins/${name}/market_chart/range?vs_currency=${currency}&from=${purchaseDate}&to=${newDate}`);
...
//       this.setState({ updatedAssetList: this.props.assets });
...
//   }
  ...
  render() {
    console.log(this.props.assets, 'props')
    return (
      <StyledAssetList>
        {this.props.assets.map((obj) => (
          <ActiveAsset key={obj.name}>
            ...
            <ListAssetInfoCont>
              <div>
                ...
                <AssetMarketPerformance>
                  <div>
                    Current Price: {getSymbolFromCurrency(obj.id)}{" "}
              ...
              <div>
                ...
                <AssetPerformance>
                  <div>
                    Coin Amount: {getSymbolFromCurrency(obj.id)}{" "}
                    {obj.assetAmount}
                  </div>
                  <div>
                    Coin Value: {getSymbolFromCurrency(obj.id)}{" "}
                    {readableNum(obj.assetAmount * obj.current_price)}
                  </div>
                  <div>Date Purchased: {obj.assetPurchaseDate}</div>
                  <div>
                    Price Change Since Purchase: {getSymbolFromCurrency(obj.id)}{" "}
